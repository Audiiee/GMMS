...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["email"]) and empty($_POST["password"])) {
        ...
        $sql = "SELECT * FROM admin WHERE Username = '$userEmail' AND Password = '$userPassword'";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            // Admin found, set session and redirect to Admin page
            $_SESSION["user"] = "admin";
            header("Location: All/Nmember.php");
            exit();
        } else {
