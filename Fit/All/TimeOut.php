<?php
  ...
  if(isset($_POST['submit'])!= ''){
    ...
    if (empty($_POST["date"]) and empty($_POST["timeOut"])) {
    ...
    $ins = "UPDATE `checkin` SET `Time_Out` = '$timeOut' WHERE `Email` = '$em' AND `Date` = '$date'";
    $result = mysqli_query($conn, $ins);

    if ($result && mysqli_affected_rows($conn) > 0) {
        $alert = "Check-Out Successfully!";
    } else {
        $alert = "I'm sorry. It's not matched on the date you've entered.";
    }
    ...
    <div class="dashboard-content px-5 pt-2">
        ...
        <div class="d-flex flex-column justify-content-center align-items-center" id="time">
            <div class="p-3 bd-highlight">
                <h2 class="fw-bold fs-2 mb-3 mt-4 text-center">CHECK-OUT</h2>
                <?php echo $alert?>
            </div>
            <div class="p-1 bd-highlight">
                <h6 class="fw-bold fa-sm">
                    To time out, please fill out the form below.
                </h6>
            </div>
            ...
            <div class="p-1 flex-grow-1 bd-highlight">
                ...
                <div class="row mb-3 text-center">
                    <div class="col-12">
                        <button class="btn btn-outline-light fw-bold btn-md" name="submit" onclick = "insertFunc()" type="submit"> Time-Out </button>
                    </div>
                </div>
            </div>
            </form>
